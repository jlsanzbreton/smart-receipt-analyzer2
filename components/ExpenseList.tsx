...
import { TrashIcon, ChevronDownIcon, ChevronUpIcon, TagIcon, CalendarDaysIcon, CurrencyEuroIcon } from '@heroicons/react/24/outline';
...
const ExpenseItem: React.FC<{ expense: CategorizedExpense; onDelete: (id: string) => void }> = ({ expense, onDelete }) => {
  ...
  return (
    <li className="bg-slate-700 shadow-lg rounded-lg overflow-hidden transition-all duration-300 hover:shadow-xl">
      ...
      >
        <div>
          <h3 className="text-lg font-semibold text-teal-400">{expense.vendorName}</h3>
      ...
      {isExpanded && (
        <div className="px-4 sm:px-6 pb-4 sm:pb-6 border-t border-slate-600">
          <h4 className="text-md font-semibold text-slate-200 mt-3 mb-2">Items:</h4>
          {expense.items && expense.items.length > 0 ? (
            <ul className="list-disc list-inside text-sm text-slate-300 space-y-1 max-h-48 overflow-y-auto pr-2" style={{ fontFamily: "'Roboto Mono', monospace"}}>
              {expense.items.map((item, index) => (
                <li key={index} className="flex justify-between">
                  <span>{item.description || 'N/A'} (Qty: {item.quantity || 1})</span>
          ...
          {expense.subtotal && <p className="text-sm text-slate-400 mt-2">Subtotal: €{expense.subtotal.toFixed(2)}</p>}
          {expense.taxAmount && <p className="text-sm text-slate-400">Tax: €{expense.taxAmount.toFixed(2)}</p>}
           {expense.paymentMethod && <p className="text-sm text-slate-400 mt-2">Payment: {expense.paymentMethod}</p>}
          <button
...
export const ExpenseList: React.FC<ExpenseListProps> = ({ expenses, onDeleteExpense }) => {
  if (expenses.length === 0) {
    return <p className="text-center text-slate-400 py-8 text-lg">No expenses recorded yet. Add one to get started!</p>;
  }
