...
self.addEventListener('fetch', (event) => {
  ...
  if (APP_SHELL_FILES.includes(url.href) || url.origin === 'https://esm.sh' || url.origin === 'https://fonts.googleapis.com' || url.origin === 'https://fonts.gstatic.com' || url.href.endsWith('.png') || url.href.endsWith('.jpg') || url.href.endsWith('.jpeg')) {
    event.respondWith(
      caches.match(event.request)
        .then((cachedResponse) => {
          ...
          return fetch(event.request).then(networkResponse => {
            ...
            if (networkResponse && networkResponse.status === 200 && (APP_SHELL_FILES.includes(url.href) || url.href === DEXIE_CDN_URL) ) { // Only cache app shell and dexie explicitly this way
        ...
        .catch(error => {
          console.error('ServiceWorker: Error fetching from cache or network:', error, event.request.url);
  ...
  } else if (event.request.method === 'GET') { // For other GET requests, try network first, then cache
    event.respondWith(
      fetch(event.request)
