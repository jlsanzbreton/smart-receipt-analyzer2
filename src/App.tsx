...
const App: React.FC = () => {
  ...
  const [savingsInsights, setSavingsInsights] = useState<
    SavingsInsightItem[] | null
  >(null);
  const [overallSavingsSummary, setOverallSavingsSummary] = useState<
    string | null
  >(null);
  ...
  const loadDataFromDB = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        `[App] Loaded ${dbExpenses.length} expenses and ${dbThresholds.length} thresholds from DB services.`
      );
  ...
  useEffect(() => {
    const migrateAndInitialize = async () => {
      ...
      console.log(
        `[App] Migration status (MIGRATION_DONE_KEY=${MIGRATION_DONE_KEY}): ${migrationDone}`
      );

      if (!migrationDone) {
        console.log(
          "[App] Migration not done. Attempting migration from localStorage to IndexedDB..."
        );
        try {
          ...
          console.log(
            `[App] Raw expenses from localStorage ('${LEGACY_LS_EXPENSES_KEY}'):`,
            lsExpensesRaw ? lsExpensesRaw.substring(0, 100) + "..." : "null"
          );
          if (lsExpensesRaw) {
            const lsExpenses = JSON.parse(
              lsExpensesRaw
            ) as CategorizedExpense[];
            if (lsExpenses.length > 0) {
              console.log(
                `[App] Found ${lsExpenses.length} expenses in localStorage. Migrating to IndexedDB...`
              );
              await db.expenses.bulkPut(lsExpenses);
              console.log(
                `[App] Successfully migrated ${lsExpenses.length} expenses to IndexedDB.`
              );
              localStorage.removeItem(LEGACY_LS_EXPENSES_KEY);
              console.log(
                `[App] Removed expenses from localStorage ('${LEGACY_LS_EXPENSES_KEY}').`
              );
            } else {
              console.log(
                "[App] No expenses found in localStorage to migrate."
              );
            }
          } else {
            console.log(
              `[App] No data found in localStorage for key '${LEGACY_LS_EXPENSES_KEY}'.`
            );
          }

          // Migrate Thresholds
          const lsThresholdsRaw = localStorage.getItem(
            LEGACY_LS_THRESHOLDS_KEY
          );
          console.log(
            `[App] Raw thresholds from localStorage ('${LEGACY_LS_THRESHOLDS_KEY}'):`,
            lsThresholdsRaw ? lsThresholdsRaw.substring(0, 100) + "..." : "null"
          );
          if (lsThresholdsRaw) {
            const lsThresholds = JSON.parse(lsThresholdsRaw) as Threshold[];
            if (lsThresholds.length > 0) {
              console.log(
                `[App] Found ${lsThresholds.length} thresholds in localStorage. Migrating to IndexedDB...`
              );
              await db.thresholds.bulkPut(lsThresholds);
              console.log(
                `[App] Successfully migrated ${lsThresholds.length} thresholds to IndexedDB.`
              );
              localStorage.removeItem(LEGACY_LS_THRESHOLDS_KEY);
              console.log(
                `[App] Removed thresholds from localStorage ('${LEGACY_LS_THRESHOLDS_KEY}').`
              );
            } else {
              console.log(
                "[App] No thresholds found in localStorage to migrate."
              );
            }
          } else {
            console.log(
              `[App] No data found in localStorage for key '${LEGACY_LS_THRESHOLDS_KEY}'.`
            );
          }

          localStorage.setItem(MIGRATION_DONE_KEY, "true");
          console.log(
            "[App] Migration process completed. MIGRATION_DONE_KEY set to true."
          );
          notify("Data successfully migrated to new offline storage.");
        } catch (err) {
          console.error(
            "[App] Error during data migration from localStorage to IndexedDB:",
            err
          );
          setError(
            "Failed to migrate old data. Some data may be missing or app may not work as expected."
          );
        ...
        console.log(
          "[App] Migration already marked as done. Skipping localStorage migration."
        );
      }

      console.log(
        "[App] Populating initial/missing thresholds in IndexedDB if necessary..."
      );
  ...
  const checkThresholds = useCallback(
    (updatedExpenses: CategorizedExpense[], currentThresholds: Threshold[]) => {
      if (!currentThresholds || currentThresholds.length === 0) return;

      const monthlyExpensesByCategory: Record<Category, number> = {} as Record<
        Category,
        number
      >;
      ...
      updatedExpenses.forEach((exp) => {
        ...
        if (
          ...
          monthlyExpensesByCategory[exp.category] =
            (monthlyExpensesByCategory[exp.category] || 0) + exp.totalAmount;
        }
      });
      ...
      currentThresholds.forEach((thr) => {
        if (
          thr.amount > 0 &&
          monthlyExpensesByCategory[thr.category] &&
          monthlyExpensesByCategory[thr.category] > thr.amount
        ) {
          notify(
            `Warning: Spending for ${
              thr.category
            } (€${monthlyExpensesByCategory[thr.category].toFixed(
              2
            )}) has exceeded your threshold of €${thr.amount.toFixed(2)}!`,
            "error"
          );
        }
      });
  ...
  useEffect(() => {
    if (expenses.length > 0 && thresholds.length > 0) {
      console.log(
        "[App] Expenses or thresholds state updated, re-checking thresholds."
      );
  ...
  const handleImageUpload = async (
    ...
    try {
      ...
      let category = await classifyExpense(
        receiptData.vendorName,
        receiptData.items.map((item) => item.description).join(", ")
      );
      ...
      await loadDataFromDB();
      notify(
        `Expense from ${newExpense.vendorName} added and categorized as ${newExpense.category}.`
      );
      ...
      const message =
        err instanceof Error
          ? err.message
          : "An unknown error occurred during receipt processing.";
      setError(message);
  ...
  const handleRequestSavingsAnalysis = async () => {
    if (expenses.length === 0) {
      setError(
        "Not enough data for savings analysis. Please add some expenses first."
      );
    ...
    try {
      ...
      const message =
        err instanceof Error
          ? err.message
          : "Failed to generate savings analysis.";
      setError(message);
      notify(message, "error");
    } finally {
      setIsLoading(false);
      ...
      case "analysis":
        return (
          <SavingsAnalysis
            ...
            overallSummary={overallSavingsSummary}
          />
        );
      case "dashboard":
      default:
  ...
  const spinnerMessage = localStorage.getItem(MIGRATION_DONE_KEY)
    ? "Processing..."
    : "Migrating data...";
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 text-slate-100 flex flex-col">
        ...
        {notification && (!error || notification !== error) && (
          <Notification
            ...
            type={
              ...
              notification.toLowerCase().includes("warning")
                ? "error"
                : "success"
            }
      ...
      <footer className="text-center p-4 text-slate-400 text-sm border-t border-slate-700">
        Smart Receipt Analyzer &copy; {new Date().getFullYear()} (Offline
        Enabled)
      </footer>
    </div>
  );
